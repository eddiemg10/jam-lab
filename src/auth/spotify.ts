...
const generateCodeVerifierChallengePair = async (): Promise<Record<string, string>> => {
...
const getAccessToken = async (code: string, codeVerifier: string): Promise<AccessToken> => {
    const params = {
        client_id: SPOTIFY_CLIENT_ID,
...
const refreshAccessToken = async (refreshToken: string): Promise<AccessToken> => {
    const params = {
        client_id: SPOTIFY_CLIENT_ID,
