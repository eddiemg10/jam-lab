...
const Track = z.object({
    album: z.object({   album_type: z.string(),   artists: z.array(Artist),   available_markets: z.array(z.string()),   external_urls: z.object({   spotify: z.string() }),   href: z.string(),   id: z.string(),   images: z.array(z.object({   height: z.number(),   url: z.string(),   width: z.number() })),   name: z.string(),   release_date: z.string(),   release_date_precision: z.string(),   total_tracks: z.number(),   type: z.string(),   uri: z.string() }),
...
const endpointBuilder = (type: TopItem, limit: number, timeRange: string) =>  `https://api.spotify.com/v1/me/top/${type}?limit=${limit}&time_range=${timeRange}`

const getTopItemsForUser = async (accessToken: string, type: TopItem, limit: number, timeRange: string): Promise<[Artist|Track]> => {
...
export { getTopItemsForUser, fetchTopArtistForMonth, fetchTopArtistsForYear, fetchTopTracksForMonth, fetchTopTracksForYear }